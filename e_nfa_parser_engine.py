...
def validate_transitions():
    ...
    for key in delta:
        if len(list(filter(lambda s: s not in list(states.keys()), delta[key].keys()))):
            return False
        if len(list(filter(lambda s: s not in sigma and s != "*", list(itertools.chain(*list(delta[key].values())))))):
            return False
...
if __name__ == "__main__":
    ...
    try:
        ...
        if validate_states() and validate_transitions() and start_state is not None and validate_road(start_state):
            print("The received automata config is a valid Lambda-NFA")
        else:
            print("The received automata config is NOT a valid Lambda-NFA")
